...
primesGeneratesPrimesProperty :: Word -> Bool
primesGeneratesPrimesProperty i = case isPrime (unPrime (primes Inf.!! i) :: GaussianInteger) of
  Nothing -> False
  Just{}  -> True
...
numberOfPrimes = assertEqual "counting primes: OEIS A091100"
  ...
  [4 * length (Inf.takeWhile ((<= 10^n) . norm . unPrime) primes) | n <- [1..6]]
