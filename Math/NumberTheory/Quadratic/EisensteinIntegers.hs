...
findPrime p = case sqrtsModPrime (9*k*k - 1) (PrimeNat . integerToNatural $ p) of
    ...
    z : _ -> EisensteinPrime $ ED.gcd (p :+ 0) ((z - 3 * k) :+ 1)
    where
        k :: Integer
        k = p `div` 6
...
primes :: [U.Prime EisensteinInteger]
primes = coerce $ (2 :+ 1) : mergeBy (comparing norm) l r
  where (leftPrimes, rightPrimes) = partition (\p -> p `mod` 3 == 2) Sieve.primes
        ...
        l = [p :+ 0 | p <- leftPrimes]
        r = [g | p <- rightPrimes', let EisensteinPrime (x :+ y) = findPrime p, g <- [x :+ y, x :+ (x - y)]]
...
factorise :: EisensteinInteger -> [(U.Prime EisensteinInteger, Word)]
factorise g = concat $
              snd $
              mapAccumL go (abs g) (Factorisation.factorise $ norm g)
  where
    go :: EisensteinInteger -> (Integer, Word) -> (EisensteinInteger, [(EisensteinPrime, Word)])
    go z (3, e) | e == n    = (q, [(EisensteinPrime (2 :+ 1), e)])
      ...
      where
        ...
        (n, q) = divideByThree z
    go z (p, e) | p `mod` 3 == 2 =
                    let e' = e `quot` 2 in (z `quotI` (p ^ e'), [(EisensteinPrime (p :+ 0), e')])
      ...
      where
        gp@(EisensteinPrime (x :+ y)) = findPrime p
        -- @gp'@ is @gp@'s conjugate.
        gp' = EisensteinPrime (x :+ (x - y))
...
divideByPrime
    :: EisensteinPrime     -- ^ Eisenstein prime @p@
    -> EisensteinPrime     -- ^ Conjugate of @p@
        ...
        go c d z = (d + d1, d + d2, z'')
            where
                ...
                z'' = head $ drop (wordToInt d2)
                    $ iterate (\g -> fromMaybe err $ (g * unEisensteinPrime p) `quotEvenI` np) z'

        go1 :: Word -> Word -> EisensteinInteger -> (Word, EisensteinInteger)
        go1 0 d z = (d, z)
        go1 c d z
            | Just z' <- (z * unEisensteinPrime p') `quotEvenI` np
            = go1 (c - 1) (d + 1) z'
            | otherwise
            = (d, z)
...
newtype EisensteinPrime = EisensteinPrime { unEisensteinPrime :: EisensteinInteger }
  deriving (Eq, Ord, Show, Generic)

instance NFData EisensteinPrime
