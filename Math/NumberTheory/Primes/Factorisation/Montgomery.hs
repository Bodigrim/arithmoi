...
factorise' :: Integer -> [(Integer, Word)]
factorise' n =
  defaultStdGenFactorisation' (mkStdGen $ fromInteger n `xor` 0xdeadbeef) n
  ...
  where
    ...
    workFact m b1 b2 count = do
      ...
      case s `modulo` fromInteger m of
        ...
        SomeMod sm ->
          case montgomeryFactorisation b1 b2 sm of
...
instance Semigroup Factors where
  Factors pfs1 cfs1 <> Factors pfs2 cfs2 = Factors (pfs1 <> pfs2) (cfs1 <> cfs2)
...
montgomeryFactorisation b1 b2 s =
  case newPoint (getVal s) n of
    Nothing -> Nothing
    Just (SomePoint p0)
...
enumAndMultiplyFromThenTo p from thn to = zip [from,thn .. to] progression
  where
    ...
    progression =
      pFrom :
      pThen : zipWith (\x0 x1 -> add x0 pStep x1) progression (tail progression)
...
list sieves =
  concat
    [ [off + toPrim i | i <- [0 .. li], unsafeIndex bs i]
    | PS vO bs <- sieves
    , let li = length bs
          off = fromInteger vO
    ]
